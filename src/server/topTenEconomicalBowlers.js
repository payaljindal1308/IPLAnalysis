...
function getTopTenEconomicBowlers(matchIds2015Obj, jsonDeliveriesArray){
    let data = jsonDeliveriesArray.reduce((bowlerData, dataRow) => {
        const { match_id, bowler } = dataRow || {};
        if(matchIds2015Obj[match_id]){
    ...
    return Object.entries(data).sort((a,b) => a[1].economyRate-b[1].economyRate).slice(0,10).reduce((bowlers, data)=> { bowlers.push(data[0])
    return bowlers},[]);
}
